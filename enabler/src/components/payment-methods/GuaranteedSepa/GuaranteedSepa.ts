...
export class GuaranteedSepa extends BaseComponent {
  ...
  async mount(selector: string) {
    ...
    setTimeout(() => {
      ...
      if (
        label &&
        label.textContent?.toLowerCase().includes("guaranteed")
      ) {
        label.textContent = "Direct Debit SEPA with payment guarantee";
      }
    ...
    if (this.showPayButton) {
      document
        .querySelector("#guaranteed-sepa-payment-button")
  ...
  private async loadNovalnetUtility(): Promise<void> {
    if (window.NovalnetUtility) {
      console.log("[Guaranteed SEPA] NovalnetUtility already loaded");
      return;
    }

    console.log("[Guaranteed SEPA] Loading NovalnetUtility...");
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      script.async = true;
      script.onload = () => {
        console.log("[Guaranteed SEPA] NovalnetUtility loaded");
        resolve();
      };
  ...
  private initializeIbanHandling() {
    const ibanInput = document.getElementById(
      "nn_guaranteesepa_account_no"
    ) as HTMLInputElement | null;

    if (!ibanInput) {
      console.warn("[Guaranteed SEPA] IBAN input not found");
      return;
    }

    const handleIban = (event: Event) => {
      console.log("[Guaranteed SEPA] IBAN changed", {
        value: ibanInput.value,
        length: ibanInput.value.length,
      });
    ...
    ibanInput.addEventListener("keyup", handleIban);
    ibanInput.addEventListener("change", handleIban);
    ibanInput.addEventListener("blur", handleIban);

    document
      .getElementById("nn_guaranteesepa_bic_div")
      ?.setAttribute(
        "style",
  ...
  async submit() {
    ...
    try {
      ...
      if (!accountHolder) {
        this.onError("Please enter account holder name.");
        return;
      }

      if (!iban) {
        this.onError("Please enter IBAN.");
        return;
      }
  ...
  private getTemplate() {
    ...
    return `
      <div class="${styles.wrapper}" style="display:flex;flex-direction:column;gap:20px;width:100%;margin-top:20px;">

        <p>Pay conveniently using Direct Debit SEPA with payment guarantee.</p>

        <div style="display:flex;flex-direction:column;gap:20px;width:100%;">

          <div style="display:flex;flex-direction:column;">
            <label for="nn_guaranteesepa_account_holder" style="font-size:14px;font-weight:600;margin-bottom:6px;">
          ...
          <div style="display:flex;flex-direction:column;">
            <label for="nn_guaranteesepa_account_no" style="font-size:14px;font-weight:600;margin-bottom:6px;">
